...
| Content | Where it lives | How hosts find it |
|---------|---------------|-------------------|
| Host helper one-pager | `hosts/[token]/index.html` | Mailmeteor email with personalized link |
...
| Content | Where it lives | How hosts find it |
|---------|---------------|-------------------|
| Scholarship status | Password-gated section of host helper | Token link (Mailmeteor) + password (separate Mailmeteor send) |
...
The delivery mechanism is Mailmeteor — the bulk personalized email tool OEFF uses. Each host receives their access credentials through the existing email cadence, not through a separate channel.

### Delivery Timeline

| Email wave | When | What's included | Mailmeteor merge fields |
|-----------|------|-----------------|------------------------|
...
### Mailmeteor Integration

Mailmeteor reads from a Google Sheet where each row = one host. The new merge fields slot into the existing Mailmeteor workflow:
...
**New merge sheet columns needed:** 4 new columns in the Mailmeteor source sheet. Each is populated by a one-time script run, not manual entry.
...
| Mailmeteor email templates | Exist, need new merge fields | Small: add 4 columns |
...
|-----------|-------------|
| Host loses their link | They reply to any OEFF email asking for it. Team re-sends the Mailmeteor email or forwards the link manually. |
